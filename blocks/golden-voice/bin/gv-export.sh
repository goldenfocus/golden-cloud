...
# Collection (output folder) defaults to "library". Override with GV_EXPORT_DIR,
# e.g. GV_EXPORT_DIR=greetings to mint terminal-greeting voices into their own set.
set -uo pipefail
BASE="$HOME/.claude/local-tts"; PY="$BASE/xtts-venv/bin/python"
COLL="${GV_EXPORT_DIR:-library}"
...
"$PY" - "$BASE/$COLL/manifest.json" "$NAME" "$TEXT" "$VOICE" "$FX" "$dur" "$lufs" "$wav" "$opus" "$mp3" <<'PY'
...
PY
echo "🎚️  exported $COLL/$NAME  (wav+opus+mp3)"

...
def stream_tts(text: str, voice_id: str, model: str, api_key: str, mode: str = MODE_FULL) -> None:
    url = (
...
def parse_args(argv: list[str]) -> tuple[str, str | None]:
    """Return (mode, explicit_text). Supports:
       bella-player.py                        → full
       bella-player.py --mode=full            → full
       bella-player.py --mode=recap           → recap
       bella-player.py --mode=say --text "…"  → say
       bella-player.py --stdin                → say, text from stdin
    """
    mode = MODE_FULL
    text: str | None = None
    read_stdin = False
...
def main() -> int:
    ...
    mode, explicit_text = parse_args(sys.argv)
    ...
    log(f"mode={mode} speaking {len(text)} chars")
    stream_tts(text, voice_id, model, api_key, mode=mode)
    return 0
